...
from app.core.session import require_authentication, get_patient_id, is_demo_patient_id
...
async def get_alerts(patient_id: str, request: Request) -> AlertDashboard:
    ...
    """
    # Demo bypass: DEMO* patients do not require a live Cerner OAuth session.
    # The recommendation engine serves them via DemoFHIRService.
    if not is_demo_patient_id(patient_id):
